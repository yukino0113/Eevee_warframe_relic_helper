...
import type { MasteryComponent, MasteryEquipment, RelicReward } from './data/primeData'
...
const localizeReward = (reward: RelicReward, language: Language, t: Translation) => `${localizePrimeItem(reward, language)} ${localizePart(reward.part, language, t)}`
...
const filterVaultedRelics = <T extends { isAya?: boolean }>(relics: T[], hideVaulted: boolean) => hideVaulted ? relics.filter((relic) => !relic.isAya) : relics
...
function App() {
  ...
  const [hideOwnedRelics, setHideOwnedRelics] = useState(() => typeof window !== 'undefined' && window.localStorage.getItem('relic-ledger-hide-owned-relics') === 'true')
  ...
  const openImport = async () => {
    const localFile = await getLocalInventoryFile()
    if (localFile) {
      await importFile(localFile, undefined, true)
      return
    }
