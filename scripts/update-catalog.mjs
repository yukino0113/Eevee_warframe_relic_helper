    ...
    for (const [mission, detail] of Object.entries(missions ?? {})) {
      ...
      for (const label of ['A', 'B', 'C']) {
        ...
        const relicRewards = rewards.filter((reward) => typeof reward?.itemName === 'string' && / Relic$/i.test(reward.itemName))
          rotations[label] = {
          chance: relicRewards.length ? capProbability(relicRewards.reduce((sum, reward) => sum + (Number(reward.chance) || 0), 0)) : null,
          relicNames: [...new Set(relicRewards.map((reward) => reward.itemName.replace(/ Relic$/i, '')))].sort(),
        }
      }
      const activeRotations = ['A', 'B', 'C'].filter((label) => rotations[label].chance !== null)
      if (!activeRotations.length) continue
      const signature = `${gameMode}::${faction}::${activeRotations.map((label) => `${label}:${rotations[label].relicNames.join(',')}:${rotations[label].chance}`).join('|')}`
      const group = groups.get(signature) ?? {
        id: normalize(signature),
        gameMode,
        faction,
        rotations: { A: { chance: null, relicNames: [] }, B: { chance: null, relicNames: [] }, C: { chance: null, relicNames: [] } },
        missions: [],
        relicNames: [],
        _samples: { A: [], B: [], C: [] },
      }
